...
tall is 1.3 % more, because 1668×2420 is the real M5 11″ panel and App Store
Connect's 11″ sizes stop at 1668×2388. That leaves no clean choice, which is
why a sync run now TODOs it: a `.monkr` canvas at the cutout size matches no
display type and the upload is refused outright, while a canvas at the App
Store size loses ~11 px off each side to `object-cover`. Embr hit the first
half of that — see *Readiness checks* below. The iMac cutout is the 4.5K
panel at 16:9
(4480×2520); the
...
excess on the other is cropped. So `anisotropy` above 0.5 % means the frame
sits between two sizes — canvas at the cutout size is refused by ASC, canvas
at the ASC size loses a strip to the crop — and someone has to choose. That
is what caught the 11″ iPad Pro.
