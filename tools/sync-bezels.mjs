...
function main() {
	...
	if (!dryRun) {
		// Skip writes whose only delta would be the timestamp — a no-op run
		// must leave the tree untouched (CI keys "did anything change?" off
		// `git status`).
		const stamp = new Date().toISOString();
		const writeStamped = (path, obj, key) => {
			const render = (ts) => JSON.stringify({ ...obj, [key]: ts }, null, '\t') + '\n';
			if (existsSync(path)) {
				try {
					const prev = readFileSync(path, 'utf8');
					if (render(JSON.parse(prev)[key]) === prev) return;
				} catch { /* unreadable/invalid → rewrite */ }
			}
			writeFileSync(path, render(stamp));
		};
		writeStamped(MANIFEST_PATH, manifest, 'updatedAt');
		writeStamped(GENERATED_PATH, {
